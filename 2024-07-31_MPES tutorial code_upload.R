...
# This .r code file presents the code used to generate the multi-parameter evidence synthesis (MPES) models in the tutorial paper.
# This file can only be executed in combination with companion .stan scripts.

# --- Start of code --- # ----
# Start run time calculations # ----
start_time <- Sys.time()
# --- Preamble --- # ----
...
pacman::p_load(flexsurv, tidyr, rstan, dplyr, survminer, survextrap, pracma, openxlsx)
# For this code to function, you will need to make sure you have a working installation of Stan.
...
# --- Data --- # ----
# Reading in data # ----
# Read in general population life table file - the below is for the UK
...
# In this section of the code, we read in the correct files based on the selected case study.
if(case_study == "SCCHN"){
  # SCCHN (head and neck)
...
# The code below is executed if running the specific scenario analysis presented in the paper for NSCLC (Figure 3, Panel C).
if (case_study == "NSCLC"){
  if (reduce_ext == TRUE){
...
# There is some tidying of the data required to set it up so that it can be used in various sections of the code that follow.
# For example, we may need to separate the treatment and control arms, and ensure treatment status is interpreted as a factor.
...
# Guyot's MPES has been converted into a single function for ease of running.
# The code below contains the function itself, including linking to the aforementioned .stan scripts.
...
} #choose whether to apply the general population constraint to the active arm (TRUE) or not (FALSE)

# Execute Guyot MPES # ----

# Code below for running the base-case models.
# Comment out and use the manual override code instead if wanting to run a custom model
...
# Manual override
# mpes_output <- run_mpes(XXXX)

guyot_end_time <- Sys.time()

# Extract Guyot MPES outputs # ----

# This section of code extracts the outputs and formats the plots for Guyot's MPES.
...
# Show Guyot MPES results # ----
survival_plot

# --- Jackson MPES --- # ----
# Execute Jackson MPES # ----

# For Jackson's MPES, we need to incorporate the ability to fit a relative survival model.
# To do this, we use a custom function called 'run_gp' which produces expected hazards based on an assumed mean start age and % female.
# There are other ways of generating expected hazards that can be used - please refer to relative survival literature.
...
# Next, we read in the external data in the format required by survextrap
...
# Using the custom 'run_gp' function, we can generate expected hazards
...
# Run the Jackson MPES model (different line executed depending on relative survival choice)
...
mod_jackson <- data.frame(time = survival(nde_mod, tmax = plot_x_limit*12, newdata = data.frame(treat = 1))$t,
                          ...
                          ucl = survival(nde_mod, tmax = plot_x_limit*12, newdata = data.frame(treat = 1))$upper)
...
mod_jackson_c <- data.frame(time = survival(nde_mod, tmax = plot_x_limit*12, newdata = data.frame(treat = 0))$t,
                            ...
                            ucl = survival(nde_mod, tmax = plot_x_limit*12, newdata = data.frame(treat = 0))$upper)
...
# Show Jackson MPES results # ----
survival_plot_jackson

# --- Comparisons --- # ----
# Compare the models # ----

# This section of code compares the outputs for Guyot's and Jackson's MPES.
...
# Same plot but without the legend for the paper
...
# We also provide comparisons of the models versus what was submitted in the two corresponding NICE appraisals.
...
if(case_study == "SCCHN"){
  ...
  ta.comp_df <- data.frame("time" = ta.comp$t_scchn,
                           ...
                           "lcl" = ta.comp$s_scchn-0.01, #not reported, so arbitrarily set here and then not plotted
                           "ucl" = ta.comp$s_scchn+0.01) #not reported, so arbitrarily set here and then not plotted
  ...
  ta.comp_df <- data.frame("time" = ta.comp$t_mel,
                           "surv" = ta.comp$s_mel,
                           "lcl" = ta.comp$s_mel-0.01, #not reported, so arbitrarily set here and then not plotted
                           "ucl" = ta.comp$s_mel+0.01) #not reported, so arbitrarily set here and then not plotted
  ...
  ta.comp_df <- data.frame("time" = ta.comp$t_nsclc,
                           ...
                           "lcl" = ta.comp$s_nsclc-0.01, #not reported, so arbitrarily set here and then not plotted
                           "ucl" = ta.comp$s_nsclc+0.01) #not reported, so arbitrarily set here and then not plotted
...
# This is a round function which maintains the number of zeros, just used for the timer
round_f <- function(x, y) {
  a <- sprintf(paste0("%.", y, "f"), round(x, y))
  return(a)
}

#Guyot run time
print(paste0("Guyot's MPES run time: ",round_f(as.numeric(difftime(guyot_end_time, guyot_start_time, units = "mins")),2)," min(s)"))

#Jackson run time
print(paste0("Jackson's MPES run time: ",round_f(as.numeric(difftime(jackson_end_time, jackson_start_time, units = "mins")),2)," min(s)"))

#Total code run time
print(paste0("Overall code run time: ",round_f(as.numeric(difftime(end_time, start_time, units = "mins")),2)," min(s)"))
