...
setwd("XXXXXXXX")
# In your working directory, you will need to create a sub-folder called 'data'
# This will need to include all the .csv files provided on the GitHub site
set.seed(99999)
...
# MPES function # ----
run_mpes <- function(single_arm_mode = FALSE, gen_pop_adjust = TRUE, hr_adjust = TRUE,
                     max_surv = plot_x_limit,
                     iter_spec = 1000,
                     gp_specs = c(34, 30196, 23080),
                     hr_specs = c(1, 0.1, 6, 28),
                     knot_loc = FALSE, #default FALSE means function will pick 2 internal knots
                     trial_source = trial.data.raw,
                     ext_source = ext.data.discrete.raw,
                     gp_choice = TRUE){
  ## Read in data
  trial.dat <- trial_source # Patient-level data from the trial
  ext.dat <- ext_source # Conditional survival data from ext registry. Data were available from 1970; as a consequence, a follow-up of 26 years was available.

  n_pred_feed <- max_surv*12
  warmup_spec <- 0.1*iter_spec #note: for now, just running this as 10% of the iterations

  if (single_arm_mode == TRUE){
    arm_select <- 0 #choose to subset the trial.dat by the flag used by treatment arm
    #arm_label <- "Placeholder" #Not sure this actually does anything
    trial.dat <- subset(trial.dat, treat == arm_select)
    trial.dat$treat <- 1 #Need to set this back to 1 for the current version of the stan code to function as intended!
  }
  ...
  logt <- log(trial.dat$t) # Create a column for log times - need this later
...
mpes_output <- run_mpes(hr_adjust = TRUE, gen_pop_adjust = TRUE, gp_specs = c(34, 30196, 23080),
                           hr_specs = c(1, 0.1, 6, 35), knot_loc = c(0, 3.39, 5.23, 6.58), iter_spec = 1000, gp_choice = TRUE)
} else if(case_study == "Melanoma"){
# Base case for melanoma
mpes_output <- run_mpes(hr_adjust = TRUE, gen_pop_adjust = TRUE, gp_specs = c(30, 45791, 41875),
                           hr_specs = c(1, 0.1, 10, 20), knot_loc = c(0, log(36), log(72), log(20*12)), iter_spec = 1000, gp_choice = TRUE)
} else {
# Base case for NSCLC
mpes_output <- run_mpes(hr_adjust = TRUE, gen_pop_adjust = TRUE, gp_specs = c(30, 45791, 41875),
                           hr_specs = c(1, 0.1, 10, 20), knot_loc = c(0, log(24), log(72), log(20*12)), iter_spec = 1000, gp_choice = TRUE)
}
...
# exploratory analysis plot code (currently not used) # ----
# fit_surv_extract_s <- subset(mpes_output_s, Strata == "treat=1")
# mod_s <- data.frame(time = fit_surv_extract_s$time,
#                     surv = fit_surv_extract_s$mean,
#                     lcl = fit_surv_extract_s$lcl,
#                     ucl = fit_surv_extract_s$ucl) #for later use
# mod_s$lbl <- "Fitted model (exploratory)"
#
# survival_plot_s <- ggplot() +
#   xlab('Time (years)') + ylab('Survival probability (%)') +
#   theme_bw() +
#   theme(axis.title.y = element_text(size = 13, angle = 90, face = "bold")) +
#   theme(axis.title.x = element_text(size = 13, angle = 0, face = "bold")) +
...
#         panel.border = element_blank(),
#         panel.background = element_blank()) +
#   scale_y_continuous(expand = c(0,0), breaks = scales::breaks_extended(n = 6), limits = c(0,100)) +
#   scale_x_continuous(expand = c(0,0), breaks = seq(0,output_plot_x_limit*12,12), limits = c(0,output_plot_x_limit*12), labels = function(x) as.integer(x / 12)) +
...
#   scale_linetype_manual(values = lines_list, breaks = legend_order) +
#   scale_fill_manual(values = fill_list, breaks = legend_order) +
#   geom_ribbon(data = km_function.long_line, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.5) +
#   geom_ribbon(data = km_function_line, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.5) +
#   geom_ribbon(data = mod, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0.5) +
#   geom_ribbon(data = mod_s, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0.5) +
#   geom_step(data = km_function.long_line, aes(x = time, y = estimate*100, col = lbl, lty = lbl), linewidth = 1) +
#   geom_step(data = km_function_line, aes(x = time, y = estimate*100, col = lbl, lty = lbl), linewidth = 1) +
#   geom_line(data = mod, aes(x = time, y = surv*100, col = lbl, lty = lbl), linewidth = 1) +
#   geom_line(data = mod_s, aes(x = time, y = surv*100, col = lbl, lty = lbl), linewidth = 1)
#
# survival_plot_s
...
if(case_study == "SCCHN"){
  ...
  # Base case for melanoma
  default_knots <- c(0, log(36), log(72), log(20*12))
  default_hr <- c(1, 0.1, 10, 20)
  default_gp <- c(30, 45791, 41875)
  s1 <- c(0, 2.48490665, 4.787491743, 6.396929655)
  s2 <- c(0, 4.094344562, 5.480638923, 6.58)
  s3 <- c(0, log(12), log(24), log(36))
  s4 <- c(0, log(72), log(20*12))
  s5 <- c(0, log(36), log(20*12))
  s6 <- c(0, log(36), log(72), log(10*12), log(20*12))
  s8 <- c(1, 0.1, 1, 20)
  s9 <- c(1, 0.1, 3, 20)
  s10 <- c(1, 0.1, 10, 11)
  s11 <- c(1, 0.1, 10, 13)
  s13 <- c(30, 458, 419)
  s14 <- c(30, 4579100, 4187500)
} else {
  # Base case for NSCLC
  default_knots <- c(0, log(24), log(72), log(20*12))
  default_hr <- c(1, 0.1, 10, 20)
  default_gp <- c(30, 45791, 41875)
  s1 <- c(0, 2.48490665, 4.787491743, 6.396929655)
  s2 <- c(0, 4.094344562, 5.480638923, 6.58)
  s3 <- c(0, log(12), log(24), log(36))
  s4 <- c(0, log(72), log(20*12))
  s5 <- c(0, log(24), log(20*12))
  s6 <- c(0, log(24), log(72), log(10*12), log(20*12))
  s8 <- c(1, 0.1, 1, 20)
  s9 <- c(1, 0.1, 3, 20)
  s10 <- c(1, 0.1, 10, 11)
  s11 <- c(1, 0.1, 10, 13)
...
# Sensitivity analyses extracting and plotting # ----
...
col_list_s <- c(
  ...
  "Original KM (control)" = "royalblue3",
  "Updated KM (control)" = "royalblue4",
  "Fitted model (SA1, active)" =   rainbow(n = 15, start = 0.1, end = 0.9)[1],
  "Fitted model (SA2, active)" =   rainbow(n = 15, start = 0.1, end = 0.9)[2],
  "Fitted model (SA3, active)" =   rainbow(n = 15, start = 0.1, end = 0.9)[3],
  "Fitted model (SA4, active)" =   rainbow(n = 15, start = 0.1, end = 0.9)[4],
  "Fitted model (SA5, active)" =   rainbow(n = 15, start = 0.1, end = 0.9)[5],
  "Fitted model (SA6, active)" =   rainbow(n = 15, start = 0.1, end = 0.9)[6],
  "Fitted model (SA7, active)" =   rainbow(n = 15, start = 0.1, end = 0.9)[7],
  "Fitted model (SA8, active)" =   rainbow(n = 15, start = 0.1, end = 0.9)[8],
  "Fitted model (SA9, active)" =   rainbow(n = 15, start = 0.1, end = 0.9)[9],
  "Fitted model (SA10, active)" =  rainbow(n = 15, start = 0.1, end = 0.9)[10],
  "Fitted model (SA11, active)" =  rainbow(n = 15, start = 0.1, end = 0.9)[11],
  "Fitted model (SA12, active)" =  rainbow(n = 15, start = 0.1, end = 0.9)[12],
  "Fitted model (SA13, active)" =  rainbow(n = 15, start = 0.1, end = 0.9)[13],
  "Fitted model (SA14, active)" =  rainbow(n = 15, start = 0.1, end = 0.9)[14],
  "Fitted model (SA15, active)" =  rainbow(n = 15, start = 0.1, end = 0.9)[15],
  "Fitted model (SA1, control)" =  rainbow(n = 15, start = 0.1, end = 0.9)[1],
  "Fitted model (SA2, control)" =  rainbow(n = 15, start = 0.1, end = 0.9)[2],
  "Fitted model (SA3, control)" =  rainbow(n = 15, start = 0.1, end = 0.9)[3],
  "Fitted model (SA4, control)" =  rainbow(n = 15, start = 0.1, end = 0.9)[4],
  "Fitted model (SA5, control)" =  rainbow(n = 15, start = 0.1, end = 0.9)[5],
  "Fitted model (SA6, control)" =  rainbow(n = 15, start = 0.1, end = 0.9)[6],
  "Fitted model (SA7, control)" =  rainbow(n = 15, start = 0.1, end = 0.9)[7],
  "Fitted model (SA8, control)" =  rainbow(n = 15, start = 0.1, end = 0.9)[8],
  "Fitted model (SA9, control)" =  rainbow(n = 15, start = 0.1, end = 0.9)[9],
  "Fitted model (SA10, control)" = rainbow(n = 15, start = 0.1, end = 0.9)[10],
  "Fitted model (SA11, control)" = rainbow(n = 15, start = 0.1, end = 0.9)[11],
  "Fitted model (SA12, control)" = rainbow(n = 15, start = 0.1, end = 0.9)[12],
  "Fitted model (SA13, control)" = rainbow(n = 15, start = 0.1, end = 0.9)[13],
  "Fitted model (SA14, control)" = rainbow(n = 15, start = 0.1, end = 0.9)[14],
  "Fitted model (SA15, control)" = rainbow(n = 15, start = 0.1, end = 0.9)[15])

lines_list_s <- c(
  "Fitted model (active)" = "dashed",
  "Fitted model (control)" = "dashed",
  "Original KM (active)" = "solid",
  "Updated KM (active)" = "solid",
  "Original KM (control)" = "dashed",
  "Updated KM (control)" = "dashed",
  "Fitted model (SA1, active)" = "dashed",
  "Fitted model (SA2, active)" = "dashed",
  "Fitted model (SA3, active)" = "dashed",
  "Fitted model (SA4, active)" = "dashed",
  "Fitted model (SA5, active)" = "dashed",
  "Fitted model (SA6, active)" = "dashed",
  "Fitted model (SA7, active)" = "dashed",
  "Fitted model (SA8, active)" = "dashed",
  "Fitted model (SA9, active)" = "dashed",
  "Fitted model (SA10, active)" = "dashed",
  "Fitted model (SA11, active)" = "dashed",
  "Fitted model (SA12, active)" = "dashed",
  "Fitted model (SA13, active)" = "dashed",
  "Fitted model (SA14, active)" = "dashed",
  "Fitted model (SA15, active)" = "dashed",
  "Fitted model (SA1, control)" = "dashed",
  "Fitted model (SA2, control)" = "dashed",
  "Fitted model (SA3, control)" = "dashed",
  "Fitted model (SA4, control)" = "dashed",
  "Fitted model (SA5, control)" = "dashed",
  "Fitted model (SA6, control)" = "dashed",
  "Fitted model (SA7, control)" = "dashed",
  "Fitted model (SA8, control)" = "dashed",
  "Fitted model (SA9, control)" = "dashed",
  "Fitted model (SA10, control)" = "dashed",
  "Fitted model (SA11, control)" = "dashed",
  "Fitted model (SA12, control)" = "dashed",
  "Fitted model (SA13, control)" = "dashed",
  "Fitted model (SA14, control)" = "dashed",
  "Fitted model (SA15, control)" = "dashed")

lwd_list_s <- c(
  "Fitted model (active)" = 2,
  "Fitted model (control)" = 2,
  "Original KM (active)" = 1.5,
  "Updated KM (active)" = 1.5,
  "Original KM (control)" = 1.5,
  "Updated KM (control)" = 1.5,
  "Fitted model (SA1, active)" = 1,
  "Fitted model (SA2, active)" = 1,
  "Fitted model (SA3, active)" = 1,
  "Fitted model (SA4, active)" = 1,
  "Fitted model (SA5, active)" = 1,
  "Fitted model (SA6, active)" = 1,
  "Fitted model (SA7, active)" = 1,
  "Fitted model (SA8, active)" = 1,
  "Fitted model (SA9, active)" = 1,
  "Fitted model (SA10, active)" = 1,
  "Fitted model (SA11, active)" = 1,
  "Fitted model (SA12, active)" = 1,
  "Fitted model (SA13, active)" = 1,
  "Fitted model (SA14, active)" = 1,
  "Fitted model (SA15, active)" = 1,
  "Fitted model (SA1, control)" = 1,
  "Fitted model (SA2, control)" = 1,
  "Fitted model (SA3, control)" = 1,
  "Fitted model (SA4, control)" = 1,
  "Fitted model (SA5, control)" = 1,
  "Fitted model (SA6, control)" = 1,
  "Fitted model (SA7, control)" = 1,
  "Fitted model (SA8, control)" = 1,
  "Fitted model (SA9, control)" = 1,
  "Fitted model (SA10, control)" = 1,
  "Fitted model (SA11, control)" = 1,
  "Fitted model (SA12, control)" = 1,
  "Fitted model (SA13, control)" = 1,
  "Fitted model (SA14, control)" = 1,
  "Fitted model (SA15, control)" = 1)
...
fill_list_s <- c(
  ...
  "Original KM (control)" = "#FFFFFF",
  "Updated KM (control)" = "#FFFFFF",
  "Fitted model (SA1, active)" = "#FFFFFF",
  "Fitted model (SA2, active)" = "#FFFFFF",
  "Fitted model (SA3, active)" = "#FFFFFF",
  "Fitted model (SA4, active)" = "#FFFFFF",
  "Fitted model (SA5, active)" = "#FFFFFF",
  "Fitted model (SA6, active)" = "#FFFFFF",
  "Fitted model (SA7, active)" = "#FFFFFF",
  "Fitted model (SA8, active)" = "#FFFFFF",
  "Fitted model (SA9, active)" = "#FFFFFF",
  "Fitted model (SA10, active)" = "#FFFFFF",
  "Fitted model (SA11, active)" = "#FFFFFF",
  "Fitted model (SA12, active)" = "#FFFFFF",
  "Fitted model (SA13, active)" = "#FFFFFF",
  "Fitted model (SA14, active)" = "#FFFFFF",
  "Fitted model (SA15, active)" = "#FFFFFF",
  "Fitted model (SA1, control)" = "#FFFFFF",
  "Fitted model (SA2, control)" = "#FFFFFF",
  "Fitted model (SA3, control)" = "#FFFFFF",
  "Fitted model (SA4, control)" = "#FFFFFF",
  "Fitted model (SA5, control)" = "#FFFFFF",
  "Fitted model (SA6, control)" = "#FFFFFF",
  "Fitted model (SA7, control)" = "#FFFFFF",
  "Fitted model (SA8, control)" = "#FFFFFF",
  "Fitted model (SA9, control)" = "#FFFFFF",
  "Fitted model (SA10, control)" = "#FFFFFF",
  "Fitted model (SA11, control)" = "#FFFFFF",
  "Fitted model (SA12, control)" = "#FFFFFF",
  "Fitted model (SA13, control)" = "#FFFFFF",
  "Fitted model (SA14, control)" = "#FFFFFF",
  "Fitted model (SA15, control)" = "#FFFFFF")
...
legend_order_s <- c(
  ...
  "Original KM (control)",
  "Updated KM (control)",
  "Fitted model (SA1, active)",
  "Fitted model (SA2, active)",
  "Fitted model (SA3, active)",
  "Fitted model (SA4, active)",
  "Fitted model (SA5, active)",
  "Fitted model (SA6, active)",
  "Fitted model (SA7, active)",
  "Fitted model (SA8, active)",
  "Fitted model (SA9, active)",
  "Fitted model (SA10, active)",
  "Fitted model (SA11, active)",
  "Fitted model (SA12, active)",
  "Fitted model (SA13, active)",
  "Fitted model (SA14, active)",
  "Fitted model (SA15, active)",
  "Fitted model (SA1, control)",
  "Fitted model (SA2, control)",
  "Fitted model (SA3, control)",
  "Fitted model (SA4, control)",
  "Fitted model (SA5, control)",
  "Fitted model (SA6, control)",
  "Fitted model (SA7, control)",
  "Fitted model (SA8, control)",
  "Fitted model (SA9, control)",
  "Fitted model (SA10, control)",
  "Fitted model (SA11, control)",
  "Fitted model (SA12, control)",
  "Fitted model (SA13, control)",
  "Fitted model (SA14, control)",
  "Fitted model (SA15, control)")

fit_surv_extract_s1 <- subset(mpes_output_s1, Strata == "treat=1")
fit_surv_extract_s2 <- subset(mpes_output_s2, Strata == "treat=1")
fit_surv_extract_s3 <- subset(mpes_output_s3, Strata == "treat=1")
fit_surv_extract_s4 <- subset(mpes_output_s4, Strata == "treat=1")
fit_surv_extract_s5 <- subset(mpes_output_s5, Strata == "treat=1")
fit_surv_extract_s6 <- subset(mpes_output_s6, Strata == "treat=1")
fit_surv_extract_s7 <- subset(mpes_output_s7, Strata == "treat=1")
fit_surv_extract_s8 <- subset(mpes_output_s8, Strata == "treat=1")
fit_surv_extract_s9 <- subset(mpes_output_s9, Strata == "treat=1")
fit_surv_extract_s10 <- subset(mpes_output_s10, Strata == "treat=1")
fit_surv_extract_s11 <- subset(mpes_output_s11, Strata == "treat=1")
fit_surv_extract_s12 <- subset(mpes_output_s12, Strata == "treat=1")
fit_surv_extract_s13 <- subset(mpes_output_s13, Strata == "treat=1")
fit_surv_extract_s14 <- subset(mpes_output_s14, Strata == "treat=1")
fit_surv_extract_s15 <- subset(mpes_output_s15, Strata == "treat=1")

fit_surv_extract_s1_c <- subset(mpes_output_s1, Strata == "treat=0")
fit_surv_extract_s2_c <- subset(mpes_output_s2, Strata == "treat=0")
fit_surv_extract_s3_c <- subset(mpes_output_s3, Strata == "treat=0")
fit_surv_extract_s4_c <- subset(mpes_output_s4, Strata == "treat=0")
fit_surv_extract_s5_c <- subset(mpes_output_s5, Strata == "treat=0")
fit_surv_extract_s6_c <- subset(mpes_output_s6, Strata == "treat=0")
fit_surv_extract_s7_c <- subset(mpes_output_s7, Strata == "treat=0")
fit_surv_extract_s8_c <- subset(mpes_output_s8, Strata == "treat=0")
fit_surv_extract_s9_c <- subset(mpes_output_s9, Strata == "treat=0")
fit_surv_extract_s10_c <- subset(mpes_output_s10, Strata == "treat=0")
fit_surv_extract_s11_c <- subset(mpes_output_s11, Strata == "treat=0")
fit_surv_extract_s12_c <- subset(mpes_output_s12, Strata == "treat=0")
fit_surv_extract_s13_c <- subset(mpes_output_s13, Strata == "treat=0")
fit_surv_extract_s14_c <- subset(mpes_output_s14, Strata == "treat=0")
fit_surv_extract_s15_c <- subset(mpes_output_s15, Strata == "treat=0")

mod_s1 <- data.frame(time = fit_surv_extract_s1$time, surv = fit_surv_extract_s1$mean, lcl = fit_surv_extract_s1$lcl, ucl = fit_surv_extract_s1$ucl)
mod_s2 <- data.frame(time = fit_surv_extract_s2$time, surv = fit_surv_extract_s2$mean, lcl = fit_surv_extract_s2$lcl, ucl = fit_surv_extract_s2$ucl)
mod_s3 <- data.frame(time = fit_surv_extract_s3$time, surv = fit_surv_extract_s3$mean, lcl = fit_surv_extract_s3$lcl, ucl = fit_surv_extract_s3$ucl)
mod_s4 <- data.frame(time = fit_surv_extract_s4$time, surv = fit_surv_extract_s4$mean, lcl = fit_surv_extract_s4$lcl, ucl = fit_surv_extract_s4$ucl)
mod_s5 <- data.frame(time = fit_surv_extract_s5$time, surv = fit_surv_extract_s5$mean, lcl = fit_surv_extract_s5$lcl, ucl = fit_surv_extract_s5$ucl)
mod_s6 <- data.frame(time = fit_surv_extract_s6$time, surv = fit_surv_extract_s6$mean, lcl = fit_surv_extract_s6$lcl, ucl = fit_surv_extract_s6$ucl)
mod_s7 <- data.frame(time = fit_surv_extract_s7$time, surv = fit_surv_extract_s7$mean, lcl = fit_surv_extract_s7$lcl, ucl = fit_surv_extract_s7$ucl)
mod_s8 <- data.frame(time = fit_surv_extract_s8$time, surv = fit_surv_extract_s8$mean, lcl = fit_surv_extract_s8$lcl, ucl = fit_surv_extract_s8$ucl)
mod_s9 <- data.frame(time = fit_surv_extract_s9$time, surv = fit_surv_extract_s9$mean, lcl = fit_surv_extract_s9$lcl, ucl = fit_surv_extract_s9$ucl)
mod_s10 <- data.frame(time = fit_surv_extract_s10$time, surv = fit_surv_extract_s10$mean, lcl = fit_surv_extract_s10$lcl, ucl = fit_surv_extract_s10$ucl)
mod_s11 <- data.frame(time = fit_surv_extract_s11$time, surv = fit_surv_extract_s11$mean, lcl = fit_surv_extract_s11$lcl, ucl = fit_surv_extract_s11$ucl)
mod_s12 <- data.frame(time = fit_surv_extract_s12$time, surv = fit_surv_extract_s12$mean, lcl = fit_surv_extract_s12$lcl, ucl = fit_surv_extract_s12$ucl)
mod_s13 <- data.frame(time = fit_surv_extract_s13$time, surv = fit_surv_extract_s13$mean, lcl = fit_surv_extract_s13$lcl, ucl = fit_surv_extract_s13$ucl)
mod_s14 <- data.frame(time = fit_surv_extract_s14$time, surv = fit_surv_extract_s14$mean, lcl = fit_surv_extract_s14$lcl, ucl = fit_surv_extract_s14$ucl)
mod_s15 <- data.frame(time = fit_surv_extract_s15$time, surv = fit_surv_extract_s15$mean, lcl = fit_surv_extract_s15$lcl, ucl = fit_surv_extract_s15$ucl)

mod_s1_c <- data.frame(time = fit_surv_extract_s1_c$time, surv = fit_surv_extract_s1_c$mean, lcl = fit_surv_extract_s1_c$lcl, ucl = fit_surv_extract_s1_c$ucl)
mod_s2_c <- data.frame(time = fit_surv_extract_s2_c$time, surv = fit_surv_extract_s2_c$mean, lcl = fit_surv_extract_s2_c$lcl, ucl = fit_surv_extract_s2_c$ucl)
mod_s3_c <- data.frame(time = fit_surv_extract_s3_c$time, surv = fit_surv_extract_s3_c$mean, lcl = fit_surv_extract_s3_c$lcl, ucl = fit_surv_extract_s3_c$ucl)
mod_s4_c <- data.frame(time = fit_surv_extract_s4_c$time, surv = fit_surv_extract_s4_c$mean, lcl = fit_surv_extract_s4_c$lcl, ucl = fit_surv_extract_s4_c$ucl)
mod_s5_c <- data.frame(time = fit_surv_extract_s5_c$time, surv = fit_surv_extract_s5_c$mean, lcl = fit_surv_extract_s5_c$lcl, ucl = fit_surv_extract_s5_c$ucl)
mod_s6_c <- data.frame(time = fit_surv_extract_s6_c$time, surv = fit_surv_extract_s6_c$mean, lcl = fit_surv_extract_s6_c$lcl, ucl = fit_surv_extract_s6_c$ucl)
mod_s7_c <- data.frame(time = fit_surv_extract_s7_c$time, surv = fit_surv_extract_s7_c$mean, lcl = fit_surv_extract_s7_c$lcl, ucl = fit_surv_extract_s7_c$ucl)
mod_s8_c <- data.frame(time = fit_surv_extract_s8_c$time, surv = fit_surv_extract_s8_c$mean, lcl = fit_surv_extract_s8_c$lcl, ucl = fit_surv_extract_s8_c$ucl)
mod_s9_c <- data.frame(time = fit_surv_extract_s9_c$time, surv = fit_surv_extract_s9_c$mean, lcl = fit_surv_extract_s9_c$lcl, ucl = fit_surv_extract_s9_c$ucl)
mod_s10_c <- data.frame(time = fit_surv_extract_s10_c$time, surv = fit_surv_extract_s10_c$mean, lcl = fit_surv_extract_s10_c$lcl, ucl = fit_surv_extract_s10_c$ucl)
mod_s11_c <- data.frame(time = fit_surv_extract_s11_c$time, surv = fit_surv_extract_s11_c$mean, lcl = fit_surv_extract_s11_c$lcl, ucl = fit_surv_extract_s11_c$ucl)
mod_s12_c <- data.frame(time = fit_surv_extract_s12_c$time, surv = fit_surv_extract_s12_c$mean, lcl = fit_surv_extract_s12_c$lcl, ucl = fit_surv_extract_s12_c$ucl)
mod_s13_c <- data.frame(time = fit_surv_extract_s13_c$time, surv = fit_surv_extract_s13_c$mean, lcl = fit_surv_extract_s13_c$lcl, ucl = fit_surv_extract_s13_c$ucl)
mod_s14_c <- data.frame(time = fit_surv_extract_s14_c$time, surv = fit_surv_extract_s14_c$mean, lcl = fit_surv_extract_s14_c$lcl, ucl = fit_surv_extract_s14_c$ucl)
mod_s15_c <- data.frame(time = fit_surv_extract_s15_c$time, surv = fit_surv_extract_s15_c$mean, lcl = fit_surv_extract_s15_c$lcl, ucl = fit_surv_extract_s15_c$ucl)

mod_s1$lbl <- "Fitted model (SA1, active)"
mod_s2$lbl <- "Fitted model (SA2, active)"
mod_s3$lbl <- "Fitted model (SA3, active)"
mod_s4$lbl <- "Fitted model (SA4, active)"
mod_s5$lbl <- "Fitted model (SA5, active)"
mod_s6$lbl <- "Fitted model (SA6, active)"
mod_s7$lbl <- "Fitted model (SA7, active)"
mod_s8$lbl <- "Fitted model (SA8, active)"
mod_s9$lbl <- "Fitted model (SA9, active)"
mod_s10$lbl <- "Fitted model (SA10, active)"
mod_s11$lbl <- "Fitted model (SA11, active)"
mod_s12$lbl <- "Fitted model (SA12, active)"
mod_s13$lbl <- "Fitted model (SA13, active)"
mod_s14$lbl <- "Fitted model (SA14, active)"
mod_s15$lbl <- "Fitted model (SA15, active)"

mod_s1_c$lbl <- "Fitted model (SA1, control)"
mod_s2_c$lbl <- "Fitted model (SA2, control)"
mod_s3_c$lbl <- "Fitted model (SA3, control)"
mod_s4_c$lbl <- "Fitted model (SA4, control)"
mod_s5_c$lbl <- "Fitted model (SA5, control)"
mod_s6_c$lbl <- "Fitted model (SA6, control)"
mod_s7_c$lbl <- "Fitted model (SA7, control)"
mod_s8_c$lbl <- "Fitted model (SA8, control)"
mod_s9_c$lbl <- "Fitted model (SA9, control)"
mod_s10_c$lbl <- "Fitted model (SA10, control)"
mod_s11_c$lbl <- "Fitted model (SA11, control)"
mod_s12_c$lbl <- "Fitted model (SA12, control)"
mod_s13_c$lbl <- "Fitted model (SA13, control)"
mod_s14_c$lbl <- "Fitted model (SA14, control)"
mod_s15_c$lbl <- "Fitted model (SA15, control)"
...
survival_plot_s <- ggplot() +
  ...
  scale_y_continuous(expand = c(0,0), breaks = scales::breaks_extended(n = 11), limits = c(0,100)) +
  scale_x_continuous(expand = c(0,0), breaks = seq(0,output_plot_x_limit*12,12), limits = c(0,output_plot_x_limit*12), labels = function(x) as.integer(x / 12)) +
  guides(col = guide_legend(title = "", nrow = legend_rows),
         linetype = guide_legend(title = "", nrow = legend_rows),
         fill = guide_legend(title = "", nrow = legend_rows),
         linewidth = guide_legend(title = "", nrow = legend_rows)) +
  scale_colour_manual(values = col_list_s, breaks = legend_order_s) +
  scale_linetype_manual(values = lines_list_s, breaks = legend_order_s) +
  scale_linewidth_manual(values = lwd_list_s, breaks = legend_order_s) +
  scale_fill_manual(values = fill_list_s, breaks = legend_order_s) +
  geom_ribbon(data = mod_s1, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s2, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s3, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s4, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s5, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s6, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s7, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s8, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s9, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s10, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s11, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s12, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s13, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s14, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s15, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s1_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s2_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s3_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s4_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s5_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s6_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s7_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s8_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s9_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s10_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s11_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s12_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s13_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s14_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = mod_s15_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = km_function.long_line, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = km_function_line, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = km_function.long_line_c, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = km_function_line_c, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = mod, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = mod_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0.2) +
  geom_line(data = mod_s1, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s2, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s3, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s4, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s5, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s6, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s7, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s8, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s9, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s10, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s11, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s12, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s13, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s14, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s15, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s1_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s2_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s3_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s4_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s5_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s6_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s7_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s8_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s9_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s10_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s11_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s12_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s13_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s14_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_s15_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_step(data = km_function.long_line, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_step(data = km_function_line, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_step(data = km_function.long_line_c, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_step(data = km_function_line_c, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data = mod_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl))

# Shows the sensitivity analyses for Guyot (full set) # ----
survival_plot_s


# Sensitivity analysis subset (key findings) # ----

survival_plot_s_sub <- ggplot() +
  xlab('Time (years)') + ylab('Survival probability (%)') +
  theme_bw() +
  theme(axis.title.y = element_text(size = 13, angle = 90, face = "bold")) +
  theme(axis.title.x = element_text(size = 13, angle = 0, face = "bold")) +
  ...
  scale_y_continuous(expand = c(0,0), breaks = scales::breaks_extended(n = 11), limits = c(0,100)) +
  scale_x_continuous(expand = c(0,0), breaks = seq(0,output_plot_x_limit*12,12), limits = c(0,output_plot_x_limit*12), labels = function(x) as.integer(x / 12)) +
  guides(col = guide_legend(title = "", nrow = legend_rows/(6/4)),
         linetype = guide_legend(title = "", nrow = legend_rows/(6/4)),
         fill = guide_legend(title = "", nrow = legend_rows/(6/4)),
         linewidth = guide_legend(title = "", nrow = legend_rows/(6/4))) +
  scale_colour_manual(values = col_list_s, breaks = legend_order_s) +
  scale_linetype_manual(values = lines_list_s, breaks = legend_order_s) +
  scale_linewidth_manual(values = lwd_list_s, breaks = legend_order_s) +
  scale_fill_manual(values = fill_list_s, breaks = legend_order_s) +
...
# Shows the sensitivity analyses for Guyot (sub set, key findings) # ----
survival_plot_s_sub

# singular plot # ----

choice_single <- mod_s6
choice_single_c <- mod_s6_c

survival_plot_s_single <- ggplot() +
  xlab('Time (years)') + ylab('Survival probability (%)') +
  theme_bw() +
  theme(axis.title.y = element_text(size = 13, angle = 90, face = "bold")) +
  theme(axis.title.x = element_text(size = 13, angle = 0, face = "bold")) +
  ...
  scale_y_continuous(expand = c(0,0), breaks = scales::breaks_extended(n = 11), limits = c(0,100)) +
  scale_x_continuous(expand = c(0,0), breaks = seq(0,output_plot_x_limit*12,12), limits = c(0,output_plot_x_limit*12), labels = function(x) as.integer(x / 12)) +
  guides(col = guide_legend(title = "", nrow = legend_rows/2),
         linetype = guide_legend(title = "", nrow = legend_rows/2),
         fill = guide_legend(title = "", nrow = legend_rows/2),
         linewidth = guide_legend(title = "", nrow = legend_rows/2)) +
  scale_colour_manual(values = col_list_s, breaks = legend_order_s) +
  scale_linetype_manual(values = lines_list_s, breaks = legend_order_s) +
  scale_linewidth_manual(values = lwd_list_s, breaks = legend_order_s) +
  scale_fill_manual(values = fill_list_s, breaks = legend_order_s) +
  geom_ribbon(data = choice_single, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = choice_single_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0) +
  geom_ribbon(data = km_function.long_line, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = km_function_line, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = km_function.long_line_c, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = km_function_line_c, aes(x = time, y = estimate*100, ymin = conf.low*100, ymax = conf.high*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = mod, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0.2) +
  geom_ribbon(data = mod_c, aes(x = time, y = surv*100, ymin = lcl*100, ymax = ucl*100, fill = lbl), alpha = 0.2) +
  geom_line(data = choice_single, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_line(data =choice_single_c, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_step(data = km_function.long_line, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_step(data = km_function_line, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_step(data = km_function.long_line_c, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
  geom_step(data = km_function_line_c, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
...
survival_plot_s_single


# Old SA code # ----

# fit_surv_extract_s6_comp <- subset(mpes_output_s6, Strata == "treat=0")
# mod_s6_comp <- data.frame(time = fit_surv_extract_s6_comp$time,
#                   surv = fit_surv_extract_s6_comp$mean,
#                   lcl = fit_surv_extract_s6_comp$lcl,
#                   ucl = fit_surv_extract_s6_comp$ucl) #for later use
# mod_s6_comp$lbl <- "Fitted model"
#
# survival_plot_s <- ggplot() +
#   xlab('Time (years)') + ylab('Survival probability (%)') +
#   theme_bw() +
...
#          linetype = guide_legend(title = "", nrow = 2),
#          fill = guide_legend(title = "", nrow = 2),
#          linewidth = guide_legend(title = "", nrow = 2)) +
#   scale_colour_manual(values = col_list_s, breaks = legend_order_s) +
#   scale_linetype_manual(values = lines_list_s, breaks = legend_order_s) +
#   scale_linewidth_manual(values = lwd_list_s, breaks = legend_order_s) +
#   geom_line(data = mod_s6, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
#   geom_line(data = mod_s6_comp, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl)) +
#   geom_step(data = km_function.long_line, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
#   geom_step(data = km_function_line, aes(x = time, y = estimate*100, col = lbl, lty = lbl, lwd = lbl)) +
#   geom_line(data = mod, aes(x = time, y = surv*100, col = lbl, lty = lbl, lwd = lbl))
...
# Compare to M-spline MPES Jackson # ----
...
nde_mod <- survextrap(Surv(t, dth) ~ treat, data=trial.data.raw,
                      chains=3, external = extdat, backhaz = mspline_data_in)
...
rmst_res <- data.frame("Analysis" = c("Base case (Guyot)", paste0("SA ", seq(1, 15)), "Base case (Jackson)"),
                       "RMST"= rep(NA, 17))

rmst_res_c <- rmst_res

rmst_res[1, 2] <- trapz(mod$time, mod$surv)
rmst_res[2, 2] <- trapz(mod_s1$time, mod_s1$surv)
rmst_res[3, 2] <- trapz(mod_s2$time, mod_s2$surv)
rmst_res[4, 2] <- trapz(mod_s3$time, mod_s3$surv)
rmst_res[5, 2] <- trapz(mod_s4$time, mod_s4$surv)
rmst_res[6, 2] <- trapz(mod_s5$time, mod_s5$surv)
rmst_res[7, 2] <- trapz(mod_s6$time, mod_s6$surv)
rmst_res[8, 2] <- trapz(mod_s7$time, mod_s7$surv)
rmst_res[9, 2] <- trapz(mod_s8$time, mod_s8$surv)
rmst_res[10,2] <- trapz(mod_s9$time, mod_s9$surv)
rmst_res[11,2] <- trapz(mod_s10$time, mod_s10$surv)
rmst_res[12,2] <- trapz(mod_s11$time, mod_s11$surv)
rmst_res[13,2] <- trapz(mod_s12$time, mod_s12$surv)
rmst_res[14,2] <- trapz(mod_s13$time, mod_s13$surv)
rmst_res[15,2] <- trapz(mod_s14$time, mod_s14$surv)
rmst_res[16,2] <- trapz(mod_s15$time, mod_s15$surv)
rmst_res[17,2] <- trapz(mod_jackson$time, mod_jackson$surv)

rmst_res_c[1, 2] <- trapz(mod_c$time, mod_c$surv)
rmst_res_c[2, 2] <- trapz(mod_s1_c$time, mod_s1_c$surv)
rmst_res_c[3, 2] <- trapz(mod_s2_c$time, mod_s2_c$surv)
rmst_res_c[4, 2] <- trapz(mod_s3_c$time, mod_s3_c$surv)
rmst_res_c[5, 2] <- trapz(mod_s4_c$time, mod_s4_c$surv)
rmst_res_c[6, 2] <- trapz(mod_s5_c$time, mod_s5_c$surv)
rmst_res_c[7, 2] <- trapz(mod_s6_c$time, mod_s6_c$surv)
rmst_res_c[8, 2] <- trapz(mod_s7_c$time, mod_s7_c$surv)
rmst_res_c[9, 2] <- trapz(mod_s8_c$time, mod_s8_c$surv)
rmst_res_c[10,2] <- trapz(mod_s9_c$time, mod_s9_c$surv)
rmst_res_c[11,2] <- trapz(mod_s10_c$time, mod_s10_c$surv)
rmst_res_c[12,2] <- trapz(mod_s11_c$time, mod_s11_c$surv)
rmst_res_c[13,2] <- trapz(mod_s12_c$time, mod_s12_c$surv)
rmst_res_c[14,2] <- trapz(mod_s13_c$time, mod_s13_c$surv)
rmst_res_c[15,2] <- trapz(mod_s14_c$time, mod_s14_c$surv)
rmst_res_c[16,2] <- trapz(mod_s15_c$time, mod_s15_c$surv)
rmst_res_c[17,2] <- trapz(mod_jackson_c$time, mod_jackson_c$surv)

# NICE STA validations # ----
